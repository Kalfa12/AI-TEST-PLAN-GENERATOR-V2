...
export function RequirementsTable({ projectId }: { projectId: string }) {
  ...
  const coveredCount = total - uncovered.size;
  const isLoading =
        ...
        <div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
          ...
          <div className="grid gap-2 md:grid-cols-[minmax(220px,1fr)_160px_150px] xl:w-[680px]">
            ...
            <select
              ...
              value={kind}
              onChange={(event) => setKind(event.target.value as RequirementKind | "all")}
            >
  ...
  return (
    <TR>
      <TD className="font-mono text-xs">
        <div className="font-medium text-foreground">{requirement.external_id ?? requirement.id}</div>
      ...
      <TD>
        <Badge tone={requirement.priority <= 2 ? "danger" : requirement.priority === 3 ? "warning" : "default"}>
      ...
      <TD>
        {uncovered ? (
          ...
          <div className="flex flex-wrap gap-1">
            <Badge tone="success">{coveredBy.length} test{coveredBy.length === 1 ? "" : "s"}</Badge>
          </div>
        )}
      </TD>
      <TD>
