...
Implements `CrossDocumentGraphProtocol`. The protocol surface is sync (matching
the in-memory NetworkX impl), so we use the synchronous neo4j driver for all
query methods. The async `close()` is the only async method.

If these methods are called from an async handler in MemoryManager, wrap with
`asyncio.to_thread(graph.add_node, ...)` for a non-blocking call path.

Install: pip install neo4j  or  pip install ai-testplan-generator[graph-neo4j]
...
try:
    ...
    raise ImportError(
        ...
        "or: pip install ai-testplan-generator[graph-neo4j]"
    ) from exc


class Neo4jCrossDocumentGraph:
    """Persistent graph store backed by Neo4j.

    All query/write methods are synchronous, matching `CrossDocumentGraphProtocol`.
    Only `close()` is async so callers can await shutdown cleanly.
    """
    ...
    def __init__(
        ...
        *,
        ...
        _log.info("neo4j_graph_init", uri=uri, database=database)
